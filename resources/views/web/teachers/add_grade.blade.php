                                 ...
                                 <form action="{{ route('grades.create') }}" method="POST">
                                    ...
                                    <div class="modal-body ">
                                       <div class="mb-2">
                                          ...
                                          <input type="text" name="gname" class="form-control fw-500 rounded-3 border-dark font-13"  >
                                       </div>
                                    </div>
...
<div class="col-lg-2 col-sm-3 mb-3 align-items-center">
    <div class="border border-info border-2 py-4 px-3 rounded-3 bd-blue-100">
        <p class="fw-bolder text-center">
            <a href="{{ url('add-subject/' . $value['id'] . '?' . http_build_query(['gname' => $value['gname']])) }}" class="hvr-shrink font-20 text-dark">{{ $value['gname'] }}</a>
        </p>
        <div class="row pt-2">
            <div class="col-lg-6">
                <!-- Edit Button -->
                <a class="" data-bs-toggle="modal" data-bs-target="#EditGrade{{ $value['id'] }}">
                    <ul class="d-flex justify-content-center font-14 text-white 
                ...
                <!-- Edit Modal -->
                <div class="modal fade" id="EditGrade{{ $value['id'] }}" data-bs-backdrop="static" 
                    tabindex="-1" aria-labelledby="editGradeLabel{{ $value['id'] }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title font-18 fw-bold text-purple pt-lg-0 pt-1" id="editGradeLabel{{ $value['id'] }}">
                                    Edit Grade
                                </h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('grades.update', $value['id']) }}" method="POST">
                                    @csrf
                                    @method('PUT') <!-- Use PUT for update -->
                                    <div class="mb-2">
                                        <label class="form-label font-14 fw-bold text-purple text-start mb-0">Edit Grade</label>
                                        <input type="text" name="gname" value="{{ $value['gname'] }}" class="form-control fw-500 rounded-3 border-dark font-13">
                                    </div>
                                    <div class="text-center">
                                        <button type="submit" class="btn text-uppercase font-12 text-white rounded-pill py-2 px-5 w-50 bg-primary fw-500">Edit</button>
                                    </div>
                                </form>
                            </div>
                        </div>
            ...
            <div class="col-lg-6">
                <!-- Delete Button -->
                <a class="" data-bs-toggle="modal" data-bs-target="#deleteGrade{{ $value['id'] }}">
                    <ul class="d-flex justify-content-center font-14 text-white 
                ...
                <!-- Delete Modal -->
                <div class="modal fade" id="deleteGrade{{ $value['id'] }}" data-bs-backdrop="static" 
                    tabindex="-1" aria-labelledby="deleteGradeLabel{{ $value['id'] }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title font-18 fw-bold text-purple pt-lg-0 pt-1" id="deleteGradeLabel{{ $value['id'] }}">
                                    Remove Grade
                                </h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('grades.destroy', $value['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE') <!-- Use DELETE for deletion -->
                                    <div class="mb-2">
                                        <p class="font-15 fw-500 text-dark pb-2">
                                            Are you sure you want to remove this grade from the system? This action cannot be undone.
                                        </p>
                                    </div>
                                    <div class="text-end">
